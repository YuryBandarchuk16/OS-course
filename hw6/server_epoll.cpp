...
public:
    ...
    server_address.sin_addr.s_addr = htonl(INADDR_ANY);
     ...
     void run() {
        ...
        int epoll_fd = epoll_create(max_conn + 5);
        if (epoll_fd < 0) {
        ...
        struct epoll_event have[max_conn + 2];
        
        struct epoll_event server_epoll;
        server_epoll.data.fd = listen_fd;
        server_epoll.events = EPOLLIN;
        epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listen_fd, &server_epoll);

        while (true) {
            int events = epoll_wait(epoll_fd, have, max_conn, -1);
            if (events < 0) {
            ...
            for (size_t i = 0; i < events; ++i) {
                if (have[i].data.fd == listen_fd) {
                    ...
                    if (rfd < 0) {
                        ...
                        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, rfd, &connection_epoll) != -1) {
                           r_fds.insert(rfd);
                        } else {
                    ...
                    int received = recv(have[i].data.fd, message, input_sz, MSG_NOSIGNAL);
                    if (received == 0 && errno != EAGAIN) {
                        shutdown(have[i].data.fd, SHUT_RDWR);
                        close(have[i].data.fd);
                    } else if (received > 0) {
                        int cur_fd = have[i].data.fd;
                        for (set<int> :: iterator it = r_fds.begin(); it != r_fds.end(); ++it) {
                            if (*it != cur_fd) {
                                send(*it, message, received, MSG_NOSIGNAL);
                            }
                        }
                    }
                }
